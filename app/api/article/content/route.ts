...
export async function GET(request: Request) {
  ...
  const generations = await getAllContentGenerations(user.id, articleId);
...
export async function POST(request: Request) {
  ...
  if (!type || !VALID_TYPES.has(type)) {
    return Response.json({ ok: false, error: "type must be one of: reel, youtube, linkedin" }, { status: 400 });
  }
  ...
  if (fetchError || !data) return Response.json({ ok: false, error: "Article not found" }, { status: 404 });

  const row = data as { title: string; summary: string; ai_summary: string; category: string };
  ...
  const { ok, error: saveError } = await saveContentGeneration(user.id, articleId, content);
  if (!ok) {
